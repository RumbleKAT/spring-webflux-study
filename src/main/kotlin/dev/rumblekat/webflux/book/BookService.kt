...
class BookService {
    ...
    val books = mutableListOf(
        Book(id = nextId.incrementAndGet(), name="kotlin in action", price = 3000),
        Book(id = nextId.incrementAndGet(), name="kotlin in action 2", price = 4000)
    )

    fun getAll() : Flux<Book> { // 복수개의 경우 flux로 리턴한다.
